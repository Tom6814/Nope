...
// maxProactiveReschedules caps how many times a gate-blocked proactive message
// may be AI-rescheduled before it is given up (防死循环).
const maxProactiveReschedules = 3
...
func parseProactiveOutcome(text string) (action proactiveAction, deferMinutes int, reason string) {
	...
	if strings.Contains(trimmed, sink.EmptyReplyMarker) {
		reason := strings.TrimSpace(strings.ReplaceAll(trimmed, sink.EmptyReplyMarker, ""))
		if reason == "" {
			reason = "AI 决定不发"
		}
		return proactiveDecline, 0, reason
	}
	// 【稍后】的结束括号可能在分钟数之后（如「【稍后 10 分钟】」），
	// 所以按前缀「【稍后」匹配，再让分钟正则去提取具体数值。
	if strings.Contains(trimmed, "【稍后") {
...
// reminderLoop periodically checks for bots that need inactivity reminders and
// due scheduled (proactive/promise) messages. The base granularity is 1 minute
// so AI-given tolerances as tight as ~2 minutes stay meaningful (design §8.3).
...
// checkScheduledMessages fires due proactive/promise messages (design §8).
// Flow per due message: anti-disturbance gate (proactive only) → 24h window
// check → live AI composition → humanized send → status update. Failures are
// recorded, never silently dropped.
func (m *Manager) checkScheduledMessages() {
	now := time.Now()
	due, err := m.store.ListDueScheduledMessages(now.Unix())
	if err != nil {
		slog.Error("scheduled message check failed", "err", err)
		return
	}
	if len(due) == 0 {
		return
	}
	cfg := persona.ResolveAIConfig(m.store)
	for i := range due {
		m.processDueScheduledMessage(&due[i], cfg, now)
	}
}

func (m *Manager) processDueScheduledMessage(msg *store.ScheduledMessage, cfg store.AIConfig, now time.Time) {
	claimed, err := m.store.ClaimScheduledMessage(msg.ID)
	if err != nil {
		slog.Error("scheduled message claim failed", "bot", msg.BotID, "id", msg.ID, "err", err)
		logScheduledResult(msg.BotID, msg.Sender, msg.Origin, "failed", "claim", "", msg.RescheduleCount)
		return
	}
	if !claimed {
		return
	...
	if msg.Tolerance > 0 && now.Unix() > msg.FireAt+msg.Tolerance {
		m.store.CancelScheduledMessageWithReason(msg.ID, "超出容差")
		logScheduledResult(msg.BotID, msg.Sender, msg.Origin, "cancelled", "tolerance", "超出容差", msg.RescheduleCount)
		return
	}

	inst, ok := m.GetInstance(msg.BotID)
	if !ok {
		m.store.MarkScheduledMessageFailed(msg.ID, "bot 不在线")
		logScheduledResult(msg.BotID, msg.Sender, msg.Origin, "failed", "offline", "", msg.RescheduleCount)
		return
	}

	// 24h WeChat iLink session window (design appendix B): outside the window
	// the push will fail regardless — mark gracefully.
	if !m.store.HasFreshContextToken(msg.BotID, 24*time.Hour) {
		m.store.MarkScheduledMessageFailed(msg.ID, "会话窗口已过期")
		logScheduledResult(msg.BotID, msg.Sender, msg.Origin, "failed", "window", "", msg.RescheduleCount)
		return
	}

	lastUserMsgAt := m.lastInboundAt(msg.BotID, msg.Sender)

	ctx := context.Background()

	// Anti-disturbance gate — only blocks origin=proactive (§8.4). When a
	// proactive message is blocked, ask the AI to pick a better time on the
	// spot (Task 6); the original row is cancelled with 重定 and a fresh
	// pending row carries the new fire_at, until the reschedule cap is hit.
	sentLog, _ := m.store.ListScheduledMessages(msg.BotID, 100)
	...
	// 热聊抑制 second opinion: recent back-and-forth may make the pending
	// proactive message pointless — the AI decides whether to still send (§8.2).
	gapNote := ""
	if msg.Origin == "proactive" && lastUserMsgAt > 0 {
		gap := now.Sub(time.Unix(lastUserMsgAt, 0))
		if gap < 30*time.Minute {
			gapNote = fmt.Sprintf("TA %s 前刚发过消息，你们可能正在聊或刚聊完", formatDuration(gap))
		}
	}

	text, outcome, err := m.aiSink.ComposeScheduledMessage(ctx, msg.BotID, msg.Sender, msg.Prompt, gapNote, inst.Provider)
	if err != nil {
		// compose-partial:产物+兜底说明已直发、仅续轮失败 —— 用户已收到内容，
		// 按 sent 记录而不是 failed（Task 7）。
		if outcome == sink.ComposeOutcomeDelivered {
			m.store.MarkScheduledMessageSent(msg.ID, now.Unix())
			logScheduledResult(msg.BotID, msg.Sender, msg.Origin, "sent", "compose-partial", err.Error(), msg.RescheduleCount)
			return
		}
	...
	case proactiveDecline:
		...
		if msg.Origin == "promise" {
			logScheduledResult(msg.BotID, msg.Sender, msg.Origin, "self-check", "ai-decline", reason, msg.RescheduleCount)
		} else {
...
// lastInboundAt returns the unix time of the person's most recent inbound
// message to the bot, or 0 if none found.
func (m *Manager) lastInboundAt(botID, sender string) int64 {
	msgs, err := m.store.ListMessagesBySender(botID, sender, 50)
	if err != nil {
		return 0
	}
	for _, ms := range msgs {
		if ms.Direction == "inbound" {
			if ms.CreateTimeMs != nil {
				return *ms.CreateTimeMs / 1000
			}
			if ms.CreatedAt > 0 {
				return ms.CreatedAt
			}
...
func (m *Manager) downloadMedia(inst *Instance, msg provider.InboundMessage, msgID int64) {
	...
	keysJSON, _ := json.Marshal(keys)
	if err := m.store.UpdateMediaStatusByID(msgID, status, keysJSON); err != nil {
