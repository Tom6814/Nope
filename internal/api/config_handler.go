...
func (s *Server) handleGetAIConfig(w http.ResponseWriter, r *http.Request) {
	...
	result := map[string]string{
		...
		"character_id":     dbConf["ai.character_id"],
		"memory_scope":     dbConf["ai.memory_scope"],
		"humanize_wechat":  dbConf["ai.humanize_wechat"],
		"proactive_enabled":     dbConf["ai.proactive_enabled"],
		"proactive_quiet_hours": dbConf["ai.proactive_quiet_hours"],
		"proactive_daily_limit": dbConf["ai.proactive_daily_limit"],
		"proactive_min_interval": dbConf["ai.proactive_min_interval"],
		"memory_consolidate_threshold": dbConf["ai.memory_consolidate_threshold"],
	}
...
func (s *Server) handleSetAIConfig(w http.ResponseWriter, r *http.Request) {
	var req struct {
		...
		HumanizeWechat         string `json:"humanize_wechat"`
		ProactiveEnabled       string `json:"proactive_enabled"`
		ProactiveQuietHours    string `json:"proactive_quiet_hours"`
		ProactiveDailyLimit    string `json:"proactive_daily_limit"`
		ProactiveMinInterval   string `json:"proactive_min_interval"`
		MemoryConsolidateThreshold string `json:"memory_consolidate_threshold"`
	}
	if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
		jsonError(w, "invalid request", http.StatusBadRequest)
		return
	...
	// Persona fields (can be cleared with empty strings).
	s.Store.SetConfig("ai.character_id", req.CharacterID)
	if req.MemoryScope != "" {
		s.Store.SetConfig("ai.memory_scope", req.MemoryScope)
	}
	if req.HumanizeWechat != "" {
		s.Store.SetConfig("ai.humanize_wechat", req.HumanizeWechat)
	}
	if req.ProactiveEnabled != "" {
		s.Store.SetConfig("ai.proactive_enabled", req.ProactiveEnabled)
	}
	if req.ProactiveQuietHours != "" {
		s.Store.SetConfig("ai.proactive_quiet_hours", req.ProactiveQuietHours)
	}
	if req.ProactiveDailyLimit != "" {
		s.Store.SetConfig("ai.proactive_daily_limit", req.ProactiveDailyLimit)
	}
	if req.ProactiveMinInterval != "" {
		s.Store.SetConfig("ai.proactive_min_interval", req.ProactiveMinInterval)
	}
	if req.MemoryConsolidateThreshold != "" {
		s.Store.SetConfig("ai.memory_consolidate_threshold", req.MemoryConsolidateThreshold)
	}
...
func (s *Server) handleDeleteAIConfig(w http.ResponseWriter, r *http.Request) {
	...
	s.Store.DeleteConfig("ai.custom_headers")
	s.Store.DeleteConfig("ai.character_id")
	s.Store.DeleteConfig("ai.memory_scope")
	s.Store.DeleteConfig("ai.humanize_wechat")
	s.Store.DeleteConfig("ai.proactive_enabled")
	s.Store.DeleteConfig("ai.proactive_quiet_hours")
	s.Store.DeleteConfig("ai.proactive_daily_limit")
	s.Store.DeleteConfig("ai.proactive_min_interval")
	s.Store.DeleteConfig("ai.memory_consolidate_threshold")
	jsonOK(w)
}
