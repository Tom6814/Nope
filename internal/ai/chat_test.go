...
type mockMessageStore struct {
	listChannelMessages       []store.Message
	listMessagesBySender      []store.Message
	listChannelMessagesCalls  int
	listMessagesBySenderCalls int
}

func (m *mockMessageStore) ListChannelMessages(channelID, sender string, limit int) ([]store.Message, error) {
	m.listChannelMessagesCalls++
	return m.listChannelMessages, nil
}
...
func (m *mockMessageStore) ListMessagesBySender(_, _ string, _ int) ([]store.Message, error) {
	m.listMessagesBySenderCalls++
	return m.listMessagesBySender, nil
}
...
func TestToolCallResult_MetadataContract_SeparatesDeliveryAndDiagnostics(t *testing.T) {
	...
	if !result.Delivery.UserFacing {
		t.Fatal("expected delivery result to be marked user-facing")
	}
	if result.Delivery.Explanation == "" {
		t.Fatal("expected default user explanation")
	}
	if result.Diagnostics.ErrorStage != "search" {
		t.Fatalf("error stage = %q, want %q", result.Diagnostics.ErrorStage, "search")
	}
	if result.Diagnostics.ErrorMessage != "timeout" {
		t.Fatalf("error message = %q, want %q", result.Diagnostics.ErrorMessage, "timeout")
	}
	if result.Diagnostics.LogFields["delivery_kind"] != "text" {
	...
	for _, key := range []string{"error_stage", "error_message"} {
		if !isReservedToolCallDiagnosticLogField(key) {
			t.Fatalf("expected %q to be treated as a reserved diagnostics field", key)
		}
		if _, ok := result.Diagnostics.LogFields[key]; ok {
			t.Fatalf("reserved diagnostics key %q must not be duplicated in LogFields", key)
		}
	}
}

func TestToolCallDiagnostics_SanitizeLogFields_StripsReservedKeys(t *testing.T) {
	diagnostics := ToolCallDiagnostics{
		ErrorStage:   "search",
		ErrorMessage: "timeout",
	...
	diagnostics.SanitizeLogFields()

	if diagnostics.LogFields["delivery_kind"] != "text" {
		t.Fatal("expected custom log field to be preserved")
	}
	for _, key := range []string{"error_stage", "error_message"} {
		if _, ok := diagnostics.LogFields[key]; ok {
