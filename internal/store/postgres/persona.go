...
func (db *DB) nowUnix() int64 { return db.now().Unix() }
...
func (db *DB) GetCharacter(id string) (*store.Character, error) {
	row := db.QueryRow(`SELECT id, name, spec, card_json, avatar_key, created_at, updated_at FROM characters WHERE id = $1`, id)
...
func scanWorldEntry(row interface{ Scan(...any) error }) (*store.WorldEntry, error) {
	...
	err := row.Scan(&e.ID, &charID, &e.Keys, &e.Content, &e.Enabled, &e.Priority, &e.CreatedAt)
	if err != nil {
		return nil, err
	}
...
func (db *DB) ListWorldEntries(characterID string) ([]store.WorldEntry, error) {
	...
	if characterID == "" {
		...
		rows, err = db.Query(`SELECT id, character_id, keys, content, enabled, priority, created_at FROM world_entries WHERE character_id = $1 OR character_id IS NULL OR character_id = '' ORDER BY priority DESC, created_at`, characterID)
	}
	if err != nil {
		return nil, err
	}
...
func (db *DB) SaveWorldEntry(e *store.WorldEntry) error {
	...
	_, err := db.Exec(`
		...
		e.ID, charID, e.Keys, e.Content, e.Enabled, e.Priority, now)
	if err != nil {
...
func (db *DB) DeleteWorldEntry(id string) error {
	_, err := db.Exec(`DELETE FROM world_entries WHERE id = $1`, id)
	return err
}

func scanContact(row interface{ Scan(...any) error }) (*store.Contact, error) {
	c := &store.Contact{}
	err := row.Scan(&c.ID, &c.BotID, &c.Sender, &c.Name, &c.Brief, &c.Profile, &c.IsOwner, &c.CreatedAt, &c.UpdatedAt)
	if err != nil {
		return nil, err
	}
	return c, nil
...
func (db *DB) UpsertContact(c *store.Contact) error {
	...
	_, err := db.Exec(`
		...
		c.ID, c.BotID, c.Sender, c.Name, c.Brief, c.Profile, c.IsOwner, now, now)
	if err != nil {
...
func (db *DB) GetContact(botID, sender string) (*store.Contact, error) {
	row := db.QueryRow(`SELECT id, bot_id, sender, name, brief, profile, is_owner, created_at, updated_at FROM contacts WHERE bot_id = $1 AND sender = $2`, botID, sender)
...
func (db *DB) ListContacts(botID string) ([]store.Contact, error) {
	rows, err := db.Query(`SELECT id, bot_id, sender, name, brief, profile, is_owner, created_at, updated_at FROM contacts WHERE bot_id = $1 ORDER BY is_owner DESC, updated_at DESC`, botID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (db *DB) SetOwner(botID, sender string) error {
	...
	defer tx.Rollback()
	if _, err := tx.Exec(`UPDATE contacts SET is_owner = FALSE, updated_at = $1 WHERE bot_id = $2 AND is_owner = TRUE`, db.nowUnix(), botID); err != nil {
		return err
	}
	if _, err := tx.Exec(`UPDATE contacts SET is_owner = TRUE, updated_at = $1 WHERE bot_id = $2 AND sender = $3`, db.nowUnix(), botID, sender); err != nil {
		return err
	}
	return tx.Commit()
}
...
func (db *DB) DeleteContact(botID, sender string) error {
	_, err := db.Exec(`DELETE FROM contacts WHERE bot_id = $1 AND sender = $2`, botID, sender)
	return err
}
...
func (db *DB) ListRelations(botID string) ([]store.ContactRelation, error) {
	rows, err := db.Query(`SELECT id, bot_id, from_sender, to_sender, relation, note, created_at, updated_at FROM contact_relations WHERE bot_id = $1 ORDER BY created_at`, botID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (db *DB) DeleteRelation(id string) error {
	_, err := db.Exec(`DELETE FROM contact_relations WHERE id = $1`, id)
	return err
}
...
func (db *DB) ListMemories(botID, sender string) ([]store.Memory, error) {
	...
	rows, err := db.Query(`SELECT id, bot_id, sender, category, content, importance, expires_at, created_at, updated_at FROM memories WHERE bot_id = $1 AND sender = $2 AND (expires_at IS NULL OR expires_at > $3) ORDER BY importance DESC, created_at DESC`, botID, sender, db.nowUnix())
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (db *DB) ListMemoriesByBot(botID string) ([]store.Memory, error) {
	rows, err := db.Query(`SELECT id, bot_id, sender, category, content, importance, expires_at, created_at, updated_at FROM memories WHERE bot_id = $1 ORDER BY importance DESC, created_at DESC`, botID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (db *DB) UpdateMemory(m *store.Memory) error {
	_, err := db.Exec(`
		UPDATE memories SET category = $1, content = $2, importance = $3, expires_at = $4, updated_at = $5 WHERE id = $6`,
		m.Category, m.Content, m.Importance, m.ExpiresAt, db.nowUnix(), m.ID)
	return err
}

func (db *DB) DeleteMemory(id string) error {
	_, err := db.Exec(`DELETE FROM memories WHERE id = $1`, id)
	return err
}

func (db *DB) CountMemories(botID, sender string) (int, error) {
	var n int
	err := db.QueryRow(`SELECT COUNT(*) FROM memories WHERE bot_id = $1 AND sender = $2`, botID, sender).Scan(&n)
	return n, err
}
...
func (db *DB) ListDueScheduledMessages(now int64) ([]store.ScheduledMessage, error) {
	rows, err := db.Query(`SELECT `+schedCols+` FROM scheduled_messages WHERE status = 'pending' AND fire_at <= $1 ORDER BY fire_at`, now)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (db *DB) ListPendingByScope(botID, sender, origin string) ([]store.ScheduledMessage, error) {
	rows, err := db.Query(`SELECT `+schedCols+` FROM scheduled_messages WHERE bot_id = $1 AND sender = $2 AND origin = $3 AND status = 'pending' ORDER BY created_at`, botID, sender, origin)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (db *DB) ClaimScheduledMessage(id string) (bool, error) {
	res, err := db.Exec(`UPDATE scheduled_messages SET status = 'processing' WHERE id = $1 AND status = 'pending'`, id)
	if err != nil {
		return false, err
	}
...
func (db *DB) MarkScheduledMessageSent(id string, sentAt int64) error {
	_, err := db.Exec(`UPDATE scheduled_messages SET status = 'sent', sent_at = $1 WHERE id = $2`, sentAt, id)
	return err
}

func (db *DB) MarkScheduledMessageFailed(id, reason string) error {
	_, err := db.Exec(`UPDATE scheduled_messages SET status = 'failed', fail_reason = $1 WHERE id = $2`, reason, id)
	return err
}

func (db *DB) CancelScheduledMessage(id string) error {
	_, err := db.Exec(`UPDATE scheduled_messages SET status = 'cancelled' WHERE id = $1`, id)
	return err
}

func (db *DB) CancelScheduledMessageWithReason(id, reason string) error {
	_, err := db.Exec(`UPDATE scheduled_messages SET status = 'cancelled', fail_reason = $1 WHERE id = $2`, reason, id)
	return err
}
...
func (db *DB) ListScheduledMessages(botID string, limit int) ([]store.ScheduledMessage, error) {
	...
	rows, err := db.Query(`SELECT `+schedCols+` FROM scheduled_messages WHERE bot_id = $1 ORDER BY created_at DESC LIMIT $2`, botID, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
func (db *DB) GetPersonaRules(botID string) (*store.PersonaRules, error) {
	...
	err := db.QueryRow(`SELECT id, bot_id, enabled, rules_text, created_at, updated_at FROM persona_rules WHERE bot_id = $1`, botID).Scan(&r.ID, &r.BotID, &r.Enabled, &r.RulesText, &r.CreatedAt, &r.UpdatedAt)
	if err == nil {
		return &r, nil
	}
	if !errors.Is(err, sql.ErrNoRows) {
	...
	// Level 2: global row.
	err = db.QueryRow(`SELECT id, bot_id, enabled, rules_text, created_at, updated_at FROM persona_rules WHERE bot_id = '' OR bot_id IS NULL`).Scan(&r.ID, &r.BotID, &r.Enabled, &r.RulesText, &r.CreatedAt, &r.UpdatedAt)
	if err == nil {
		return &r, nil
	}
	if !errors.Is(err, sql.ErrNoRows) {
...
func (db *DB) SavePersonaRules(r *store.PersonaRules) error {
	...
	_, err := db.Exec(`
		...
		r.ID, r.BotID, r.Enabled, r.RulesText, now, now)
	if err != nil {
