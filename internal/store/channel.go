...
type AIConfig struct {
	...
	// --- Persona / memory (see docs/design) ---
	// CharacterID selects the bot-level character card ("ai.character_id").
	CharacterID string `json:"character_id,omitempty"`
	// MemoryScope is one of "global" (default), "current", "freeform".
	MemoryScope string `json:"memory_scope,omitempty"`
	// HumanizeWechat splits multi-line replies into short WeChat-style bursts.
	HumanizeWechat bool `json:"humanize_wechat,omitempty"`

	// --- Proactive messaging guards (only apply to origin=proactive) ---
	ProactiveEnabled     bool   `json:"proactive_enabled,omitempty"`
	ProactiveQuietHours  string `json:"proactive_quiet_hours,omitempty"` // "HH:MM-HH:MM", e.g. "23:00-08:00"
	ProactiveDailyLimit  int    `json:"proactive_daily_limit,omitempty"`
	ProactiveMinInterval int    `json:"proactive_min_interval,omitempty"` // seconds

	// MemoryConsolidateThreshold triggers an async consolidation when a
	// (bot, sender) memory count exceeds it.
	MemoryConsolidateThreshold int `json:"memory_consolidate_threshold,omitempty"`
}
