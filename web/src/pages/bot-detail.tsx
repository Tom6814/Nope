...
import { botDisplayName, api } from "../lib/api";
import {
  useBot,
  useBotApps,
  useDeleteBot,
...
function scheduledStatusBadge(status: string) {
  switch (status) {
    case "sent":
      return (
    ...
    default:
      return <Badge>待发送</Badge>;
  }
}

function ScheduledMessagesCard({ botId }: { botId: string }) {
  ...
  const handleCancel = async (m: any) => {
    ...
    setCancelling(m.id);
    try {
      await api.cancelScheduledMessage(botId, m.id);
      toast({ title: "已取消" });
      load();
    } catch (e: any) {
      toast({ variant: "destructive", title: "取消失败", description: e.message });
    } finally {
      setCancelling(null);
    }
  };
  ...
  const summaryBadges = [
    { key: "pending", label: "待发送", variant: "default" as const },
    {
      key: "sent",
      label: "已发送",
    ...
    { key: "failed", label: "失败", variant: "destructive" as const },
    {
      key: "cancelled",
      label: "已取消",
      variant: "outline" as const,
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-sm font-semibold text-muted-foreground">定时/主动消息</h2>
      <Card>
        {ConfirmDialog}
        <CardHeader className="pb-0">
          <div className="flex items-center justify-between gap-2">
            <CardTitle className="text-sm">近 7 天概览</CardTitle>
            {!loading && !error ? (
              ...
              <div className="flex items-center gap-2 flex-wrap">
                {summaryBadges.map((b) => (
                  <Badge key={b.key} variant={b.variant} className={b.className}>
                    {b.label} {byStatus[b.key] ?? 0}
                  </Badge>
                ))}
              </div>

              {recent.length === 0 ? (
                <div className="text-center py-10 space-y-2 border-2 border-dashed rounded-xl">
                  <Clock className="w-8 h-8 mx-auto text-muted-foreground/40" />
                  <p className="text-sm text-muted-foreground">还没有定时/主动消息</p>
                </div>
              ) : (
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className="text-center">重排</TableHead>
                    ...
                    {recent.map((m) => (
                      <TableRow key={m.id}>
                        <TableCell className="whitespace-nowrap text-xs">
                          {formatRelativeTime(m.fire_at)}
                        </TableCell>
                        <TableCell>
                          <Badge
                            variant={m.origin === "proactive" ? "secondary" : "outline"}
                            className="text-[10px]"
                          >
                            {m.origin === "proactive" ? "主动" : "承诺"}
                          </Badge>
                        </TableCell>
                        <TableCell>{scheduledStatusBadge(m.status)}</TableCell>
                        ...
                        <TableCell className="text-xs whitespace-nowrap">
                          {m.sent_at ? new Date(m.sent_at * 1000).toLocaleString() : "—"}
                        </TableCell>
                        <TableCell>
                          {m.status === "pending" ? (
                            <Button
...
export function BotDetailPage() {
  ...
  return (
    <div className="flex flex-col gap-8 h-full">
      ...
      <ScheduledMessagesCard botId={id!} />
    </div>
  );
}
